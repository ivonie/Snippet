...
public class brightPicker extends View{
    ...
    private float brightHSV = 0f;
    private double selectedBright;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText(state, centerX- sizeLargeText, centerY+(getResources().getDimensionPixelSize(R.dimen.texto_normal)/2), buttonTextPaint);
    ...
    private void drawPointer(Canvas canvas) {
        ...
        double hueAngle = (float) Math.toRadians(brightHSV);
    ...
    private void moverThumb(MotionEvent event){
        ...
        if (d <= brightWheelRadius & d > buttonWheelRadius) {
            brightHSV = (float) (Math.toDegrees(Math.atan2(cy, cx)));
            invalidate();
            selectedBright = ((-Math.sin(Math.toRadians(brightHSV)) + 1 ) /2 ) * scale;
            if (selectedBright > (180/ scale)) {
                onOff = true;
                state = "ON";
                sizeLargeText = getResources().getDimensionPixelSize(R.dimen.texto_normal)*2/3;
            }
    ...
    public void setBright(int brillo, int escala) {
        this.scale = escala;
        double aux = (double) brillo / escala;
        double rad = - (2 * aux ) + 1;
        brightHSV = (float) rad * 90;
        if (brightHSV == 90){
            onOff = false;
            state = "OFF";
            sizeLargeText = getResources().getDimensionPixelSize(R.dimen.texto_normal)*4/5;
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        state.putFloat("color", brightHSV);
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            brightHSV = bundle.getFloat("color");
